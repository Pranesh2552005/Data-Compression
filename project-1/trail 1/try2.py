...
# Print the lengths with a maximum of 15 characters per line
character_count = 0
for length in lengths_of_result_strings:
    if character_count + len(str(length)) <= 15:
        print(length, end=' ')
        character_count += len(str(length)) + 1  # Add 1 for the space
    else:
        print()  # Move to the next line
        print(length, end=' ')
        character_count = len(str(length)) + 1  # Reset character count
